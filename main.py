...
# Loads all the environmental variables from the .env
load_dotenv()
...
class BlogPost(db.Model):
    ...
    # Parents relationship with child(Comment)
    blog_comments = relationship('Comment', back_populates='parent_blog')
...
# a decorator function that allows for admins only to visit certain parts of the website
def admin_only(func):
    @wraps(func)
    def decorater_func(*args, **kwargs):
        if current_user.id != 1:
            return abort(403)
        site = func(*args, **kwargs)
        return site
...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        if email:
            flash('This email already exists. Consider login instead.')
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            flash("That email doesn't exist in our system.")
            return redirect(url_for('register'))
        # if the password entered is incorrect
        elif  not check_password_hash(user.password, password):
            flash("You entered a wrong password")
            return redirect(url_for('login'))
        else:
            login_user(user)
...
def contact():
    if request.method == 'POST':
        ...
        print(user_name, email_add, phone_num, msg)
        sendmail(username=user_name, email=email_add, message=msg, users_phone_number=phone_num)
